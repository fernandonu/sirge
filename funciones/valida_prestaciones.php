...
if ($padron = fopen ($archivo , "rb")) {
	...
	*/
		...
		if ($linea_actual > 1) {
			...
			*/
			if (count ($campos) <> $campos_prestaciones) {
				$rechazado = 1;
				$motivo .= 'No cumple con los campos mínimos requeridos';
			} else {
				...
				*/
				...
				if (strlen ($campos[3]) > 9) {
					$rechazado = 1;
					$motivo .= 'Prestación inválida';
				}
			...
			*/
			if ($rechazado) {
				...
				*/
				//file_put_contents ("../upload/prestaciones/rec/rechazados-" . date("m") . "-P-" . $provincia . ".txt" , $motivo . ' en línea ' . $linea_actual . ';' . implode (";" , $campos) . "\r\n" , FILE_APPEND);
				...
				$sql = "INSERT INTO prestaciones.p_" . $provincia . " VALUES ('" . implode ("','" , $campos) . "', $lote[0])";
				$res = pg_query ($sql);
				
				if (!$res) {
					//echo $sql , "<br /> Se ha producido un error " . pg_last_error();
					$motivo .= "Sintáxis inválida";
					file_put_contents ("../upload/prestaciones/rec/rechazados-" . date("m") . "-P-" . $provincia . ".csv" , $motivo . ' en línea ' . $linea_actual . ';' . implode (";" , $campos) . "\r\n" , FILE_APPEND);
					$cantidad_rechazados += 1;
				} else {
					$cantidad_insertados += 1;
				}
			}
		}
	 }
	...
	echo "
		Total de registros 		: " , $padron_lineas - 1 , " <br />
