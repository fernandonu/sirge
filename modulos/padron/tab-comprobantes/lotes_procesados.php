...
$sql = "
	...
	from (
		...
		from 
			...
			left join sistema.cargas_archivos c on l.lote = c.lote";
...
$(document).ready(function(){
		...
		$("#dialog-confirm").dialog({
			...
			buttons: {
				"Aceptar" : function () {
					...
					$.ajax({
						type : 'post' ,
						data : 'acc=B&padron=comprobantes&lote=' + lote + '&provincia=' + provincia + '&padron=comprobantes' ,
						...
						success : function (data) {
							...
							$("#dialog-respuesta").dialog({
								...
								buttons: {
									Ok: function() {
										$("#listado-lotes").load("modulos/padron/tab-comprobantes/lotes_procesados.php #listado-lotes" , function () {
											$("dd").hide();
										});
										$(this).dialog("close");
									}
		...
		$("#dialog-confirm").dialog({
			...
			buttons: {
				"Aceptar" : function () {
					...
					$.ajax({
						type : 'post' ,
						data : 'acc=C&lote=' + lote + '&provincia=' + provincia + '&padron=comprobantes' ,
						url  : 'funciones/abm_lotes.php' ,
						success : function (data) {
							$("#dialog-respuesta").html(data);
							$("#dialog-respuesta").dialog({
	...
	$("#listado-lotes").on("click" , ".listado-lotes-paginado span" , function(event){
		...
		$("#listado-lotes").load("modulos/padron/tab-comprobantes/lotes_procesados.php?pagina=" + page + " #listado-lotes" , function(){
			$("dd").hide();
		});
	});
	...
	$("#listado-lotes").on("click" , ".imprimir-ddjj" , function (event){
		...
		$.ajax({
			...
			success : function () {
				window.location.href = 'funciones/genera_declaracion_jurada.php?lote=' + lote + '&provincia=' + provincia + '&padron=3';
			}
		});
	});
	...
	$("form").submit(function(event){
		event.preventDefault();
		$("#listado-lotes").load('modulos/padron/tab-comprobantes/lotes_procesados.php?' + $(this).serialize() + ' #listado-lotes' , function(){
			$("dd").hide();
		});
	});
	...
	$("#listado-lotes").on("click" , ".registros-rechazados" , function(){
		...
		$.ajax ({
			type : 'get' ,
			url  : 'modulos/consultas/consulta_lotes_rechazados.php?padron=3' ,
			data : 'lote=' + lote ,
			success : function (data) {
				$.fancybox(data);
			}
